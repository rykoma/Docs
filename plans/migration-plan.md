...
- [x] アクセシビリティとレスポンシブ表示を確認した (キーボード フォーカス順序・可視化をヘッダー / メイン領域 / 右サイドバー / フッターの全域で確認・改善、レスポンシブ表示・色コントラスト・画像 alt・言語切り替えの一貫性を確認。スクリーンリーダー対応は今回の移行作業の対象外とする)
- [ ] コードブロックのデザインと機能 (シンタックス ハイライト、折り返し、コピー操作など) を確認した
- [ ] 記事の共有リンクの動作とデザインを確認した
- [ ] 本文へのスキップ リンクの追加を検討した
- [ ] 404 ページを作成した
- [ ] モバイル表示でのタッチ ターゲット サイズ (リンク・ボタンの目安 44×44px) を確認した
