...
- [x] ブラウザーの拡大表示 (200% 目安) でヘッダー・サイドバー・記事本文が崩れないことを確認した
- [x] 検索フォームなどのフォーム要素で label と入力欄が適切に関連付けられているか確認した (ヘッダーの検索フォームを Bing 検索向けに自前実装へ置き換え、検索入力欄に `label` と `id` / `for` の関連付けを追加。送信ボタンにはアクセシブルな名前とキーボード フォーカス表示を追加し、Enter、虫眼鏡ボタン、Tab、Shift+Tab、フォーカス喪失時の開閉、モバイル横スクロール対策への影響を確認した)
